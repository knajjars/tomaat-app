...
exports.config = function(input) {
  if (input.app_id && input.app_key) {
    ...
    Config.app_id_query =
      "?_app_id=" + input.app_id + "&_app_key=" + input.app_key;
  }
};
  ...
  return {
    ...
    allowedCuisines: function(input) {
      if (input && input.length) {
        ...
        if (input instanceof Array) {
          input.forEach(function(item) {
            url += "&allowedCuisine[]=" + encodeURIComponent(item);
            settings.allowedCuisines.push(item);
          });
        } else if (typeof input == "string") {
          url +=
            "&allowedCuisine[]=cuisine^cuisine-" + encodeURIComponent(input);
    ...
    excludedCuisines: function(input) {
      if (input && input.length) {
        ...
        if (input instanceof Array) {
          input.forEach(function(item) {
            url +=
              "&excludedCuisine[]=cuisine^cuisine-" + encodeURIComponent(item);
            settings.excludedCuisines.push(item);
          });
        } else if (typeof input == "string") {
          url +=
            "&excludedCuisine[]=cuisine^cuisine-" + encodeURIComponent(input);
    ...
    excludedCourses: function(input) {
      if (input && input.length) {
        ...
        if (input instanceof Array) {
          input.forEach(function(item) {
            url +=
              "&excludedCourse[]=course^course-" + encodeURIComponent(item);
        ...
        if (input instanceof Array) {
          for (var i = 0; i < input.length; i++) {
            url +=
              ...
              encodeURIComponent(input[i]);
            settings.allowedHolidays.push(input[i]);
          }
        } else if (typeof input == "string") {
          url +=
            "&allowedHoliday[]=holiday^holiday-" + encodeURIComponent(input);
    ...
    excludedHolidays: function(input) {
      if (input && input.length) {
        ...
        if (input instanceof Array) {
          input.forEach(function(item) {
            url +=
              "&excludedHoliday[]=holiday^holiday-" + encodeURIComponent(item);
            settings.excludedHolidays.push(item);
          });
        } else if (typeof input == "string") {
          url +=
            "&excludedHoliday[]=holiday^holiday-" + encodeURIComponent(input);
...
exports.getMeta = function(input, display) {
  ...
  fs.stat(path, function(err, stat) {
    if (err == null) {
      ...
      fetchMeta(url, path, input, display)
        .then(function(resp) {
          // console.log(resp);
